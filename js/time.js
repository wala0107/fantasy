...
var PRESTimeline = /** @class */function () {
  ...
  PRESTimeline.prototype._parseData = function () {
    ...
    for (var _i = 0, periods_1 = periods; _i < periods_1.length; _i++) {if (window.CP.shouldStopExecution(0)) break;
      var section = periods_1[_i];
      section.period = $(section).attr('period');
      section.index = $(section).index();
    }
    // console.log(periods)
    window.CP.exitedLoop(0);this.periodData = periods;
    var data = $(base).find('.cards-container section');
    // console.log(data)
    for (var _a = 0, data_1 = data; _a < data_1.length; _a++) {if (window.CP.shouldStopExecution(1)) break;
      var section = data_1[_a];
      section.period = $(section).attr('period');
      section.index = $(section).index();
    }
    // console.log(data)
    window.CP.exitedLoop(1);this.cardData = data;
  ...
  PRESTimeline.prototype._assignBtn = function () {
    ...
    for (var i = 0; i < this.timelineData.length; i++) {if (window.CP.shouldStopExecution(3)) break;
      _loop_1(i);
    }window.CP.exitedLoop(3);
  };
  // ## color ##
  PRESTimeline.prototype._initialColor = function () {
    for (var i = 0; i < this.periodData.length; i++) {if (window.CP.shouldStopExecution(4)) break;
    ...
    for (var i = 0; i < this.cardData.length; i++) {if (window.CP.shouldStopExecution(5)) break;
    ...
    switch (state) {
      case 'period':
        ...
        if (this.activePeriod.period != this.activeCard.period) {
          ...
          var ta = [];
          for (var i = 0; i < this.cardData.length; i++) {if (window.CP.shouldStopExecution(6)) break;
            var temp = this.cardData[i];
      ...
      case 'timeline':
        ...
        if (this.activeCard.period != this.activePeriod.period) {
          var ta = void 0;
          for (var i = 0; i < this.periodData.length; i++) {if (window.CP.shouldStopExecution(7)) break;
            var temp = this.periodData[i];
